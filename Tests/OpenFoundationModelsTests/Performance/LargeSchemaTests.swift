...
struct LargeSchemaTests {
    ...
    func largeSchemaWithManySimpleProperties() {
        ...
        let schema = LargeUserProfile.generationSchema
    ...
    func multipleLargeSchemasInParallel() async {
        ...
        async let productSchema = Task { ProductCatalog.generationSchema }
    ...
    func schemaMemoryEfficiencyWithRepeatedAccess() {
        ...
        for _ in 0..<iterations {
            let schema = RepeatedAccessType.generationSchema
            #expect(schema.type == "object")
        }
